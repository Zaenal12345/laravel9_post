...
<div class="page-wrapper">
    <div class="page-content">
        ...
        <!--end breadcrumb-->
        <form action="{{ route('good.update', $goods->id) }}" method="post" id="goodsForm">
        @csrf
        @method('PUT')
        <div class="row">
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <div>
                            ...
                            <select class="form-select mb-2" name="category_id" id="category_id">
                                @foreach($category as $data)
                                    <option value="{{ $data->id }}" <?= $data->id == $goods->category_id ? "selected" : "" ?>>{{ $data->category_name }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <input class="form-control mb-2" type="text" placeholder="" name="goods_name" id="goods_name" value="{{$goods->goods_name}}">
                        </div>

                        <div>
                            <label for="price" class="form-label"><b>Harga :</b></label>
                            <input class="form-control mb-2" type="text" placeholder="" name="price" id="price" value="{{$goods->price}}">
                        </div>
                        
                        <div>
                            <label for="unit" class="form-label"><b>Satuan :</b></label>
                            <select class="form-select mb-2" name="unit" id="unit">
                                <option value="pcs" <?= $goods->unit == "pcs" ? "selected" : "" ?>>pcs</option>
                                <option value="buah" <?= $goods->unit == "buah" ? "selected" : "" ?>>buah</option>
                            </select>
                        </div>
                        
                        <div>
                            <label for="mininum_stock" class="form-label"><b>Stok Minimum :</b></label>
                            <input class="form-control mb-2" type="text" placeholder="" name="mininum_stock" id="mininum_stock" value="{{$goods->mininum_stock}}">
                        </div>
                        
                        <div>
                            <label for="expired_date" class="form-label"><b>Tanggal Kadaluarsa :</b></label>
                            <input class="form-control mb-2" type="date" name="expired_date" id="expired_date" value="{{$goods->expired_date}}">
                        </div>
